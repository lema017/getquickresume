/**
 * name: gqr-resume-mist
 * description: "Variación del estilo gris minimalista: encabezado ‘paper’ con borde y acento sutil, títulos de sección con etiqueta (pill) y línea fina, entradas con borde izquierdo marcado; look editorial limpio y elegante."
 */
(function () {
    'use strict';
    ...
    class GQRResumeMist extends HTMLElement {
      ...
      renderSkills() {
        ...
        const grid = skills
          .map((s, i) => `<div class="skill" data-entry-id="skill-${i}">• ${escapeHtml(s)}</div>`)
          .join('');
      ...
      // Variation title: pill label + hairline
      renderTitle(label) {
        return `
          <div class="title">
            <span class="title-pill">${escapeHtml(label)}</span>
            <span class="title-hair" aria-hidden="true"></span>
          </div>
        `;
      }
      ...
      render() {
        ...
        this.shadowRoot.innerHTML = `
          <style>
            ...
            /* Header variation: "paper card" with subtle accent */
            .hero {
              background: #f1f1f1;
              border-radius: 16px;
              padding: 20px 22px;
            ...
            /* Skills variation: 2-column grid */
            .skill-grid {
              display: grid;
            ...
            /* Entry variation: left border accent */
            .entry {
              background: #ffffff;
              border: 1px solid #ededed;
              border-radius: 12px;
    ...
    if (!customElements.get('gqr-resume-mist')) {
      customElements.define('gqr-resume-mist', GQRResumeMist);
    }
  })();