/**
 * name: gqr-resume-executive
 * description: "Single-column resume with dark navy header banner split (name left, contact right), navy section bars, gold accent on skills bar, two-column education/skills grids, and clean corporate layout."
 */
...
(function() {
  ...
  var i18n = {
    es: {
      profile: 'Acerca de mí', experience: 'Experiencia Laboral',
    ...
    en: {
      profile: 'About Me', experience: 'Work Experience',
  ...
  class GQRResumeExecutive extends HTMLElement {
    ...
    render() {
      ...
      var css = '<style>' +
        ...
        /* Gold section bar (for skills) */
        '.sec-bar-gold {' +
        '  background: ' + gold + ';' +
        '  padding: 5px 14px;' +
        '  margin-bottom: 10px;' +
        '}' +
        '.sec-bar-gold .sec-bar-title {' +
        '  color: #fff;' +
        '}' +
      ...
      function secBarGold(label) {
        return '<div class="sec-bar-gold"><span class="sec-bar-title">' + escapeHtml(label) + '</span></div>';
      }

      var html = css + '<div class="page">';

      /* Header banner - split: name left, contact right */
      ...
      if (hasContact) {
        ...
        if (phone) html += '<div class="hdr-ct-item"><span class="hdr-ct-icon">☎</span> ' + escapeHtml(phone) + '</div>';
        if (email) html += '<div class="hdr-ct-item"><span class="hdr-ct-icon">✉</span> ' + escapeHtml(email) + '</div>';
        if (country) html += '<div class="hdr-ct-item"><span class="hdr-ct-icon">⚲</span> ' + escapeHtml(country) + '</div>';
        if (linkedin) html += '<div class="hdr-ct-item"><span class="hdr-ct-icon">🔗</span> ' + escapeHtml(linkedin) + '</div>';
        html += '</div>';
      }
      html += '</div>'; // header-banner
      ...
      if (experience.length > 0) {
        ...
        for (var ei = 0; ei < experience.length; ei++) {
          ...
          var companyLine = [];
          if (company) companyLine.push(company);
          if (location) companyLine.push(location);
          if (companyLine.length) html += '<div class="exp-company">' + escapeHtml(companyLine.join(', ')) + '</div>';
          html += '</div>';
  ...
  if (!customElements.get('gqr-resume-executive')) {
    customElements.define('gqr-resume-executive', GQRResumeExecutive);
  }
})();
