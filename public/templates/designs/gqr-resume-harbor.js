/**
 * name: gqr-resume-harbor
 * description: "Estilo moderno con banda superior azul oscuro y tipografía en mayúsculas, secciones separadas por líneas finas y layout en una sola columna que imita un diseño editorial (títulos con tracking amplio, bloques compactos y acento dorado sutil)."
 */
(function () {
    'use strict';
    ...
    class GQRResumeHarbor extends HTMLElement {
      ...
      render() {
        ...
        // 1) Header
        const headerHtml = `
          <div class="topbar" data-section="header">
        ...
        // We already opened .content in header; to keep DOM simple, we will render the rest in a second content block
        const bodyOpen = `<div class="content">`;
        const bodyClose = `</div>`;
                    ...
                    return `
                      <div class="entry" data-entry-id="${escapeHtml(id)}">
                        <div class="entry-head">
                          <div>
                            ${name ? `<div class="entry-title">${escapeHtml(name)}</div>` : ''}
                            ${right ? `<div class="entry-sub">${escapeHtml(right)}</div>` : ''}
                          </div>
                        </div>
                      </div>
                    `;
    ...
    if (!customElements.get('gqr-resume-harbor')) {
      customElements.define('gqr-resume-harbor', GQRResumeHarbor);
    }
  })();