/**
 * name: gqr-resume-mckinsey
 * description: "Traditional consulting-style single-column resume with centered header, thin horizontal rule section dividers, bold inline labels, right-aligned dates and locations, and serif-free professional typography."
 */
...
(function() {
  ...
  const i18n = {
    es: {
      profile: "RESUMEN PROFESIONAL", experience: "EXPERIENCIA PROFESIONAL",
    ...
    en: {
      profile: "PROFESSIONAL SUMMARY", experience: "PROFESSIONAL EXPERIENCE",
  ...
  class GQRResumeMckinsey extends HTMLElement {
    constructor() {
      super();
      this._data = null;
    }
    ...
    render() {
      ...
      var styles = '<style>' +
        ...
        '  white-space: nowrap;' +
        '}' +
        '.hc-icon { font-size: 9px; color: #888; }' +
        ...
        '  margin-top: 1px;' +
        '}' +
        '.edu-bullets { list-style: none; margin-top: 3px; }' +
        '.edu-bullet {' +
        '  font-size: 11.5px;' +
        '  line-height: 1.5;' +
        '  color: #333;' +
      ...
      var cItems = [];
      if (country) cItems.push('<span class="header-contact-item"><span class="hc-icon">⚲</span>' + escapeHtml(country) + '</span>');
      if (email) cItems.push('<span class="header-contact-item"><span class="hc-icon">✉</span>' + escapeHtml(email) + '</span>');
      if (phone) cItems.push('<span class="header-contact-item"><span class="hc-icon">☎</span>' + escapeHtml(phone) + '</span>');
      if (linkedin) cItems.push('<span class="header-contact-item"><span class="hc-icon">🔗</span>' + escapeHtml(linkedin) + '</span>');
      if (cItems.length > 0) {
      ...
      if (summary) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.profile) + '</div>';
      ...
      if (skills.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.skills) + '</div>';
      ...
      if (experience.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.experience) + '</div>';
        for (var ei = 0; ei < experience.length; ei++) {
          ...
          html += '<span class="exp-title">' + escapeHtml(position) + '</span>';
          if (expRange) html += '<span class="exp-right"><span class="exp-date">' + escapeHtml(expRange) + '</span></span>';
          html += '</div>';

          if (company) {
            html += '<div class="exp-row">';
            html += '<span class="exp-company">' + escapeHtml(company) + '</span>';
            if (country) html += '<span class="exp-right"><span class="exp-location">' + escapeHtml(country) + '</span></span>';
            html += '</div>';
          }
      ...
      if (projects.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.projects) + '</div>';
      ...
      if (achievements.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.achievements) + '</div>';
      ...
      if (education.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.education) + '</div>';
        for (var edi = 0; edi < education.length; edi++) {
          ...
          if (eduRange) html += '<span class="edu-right"><span class="edu-date">' + escapeHtml(eduRange) + '</span></span>';
          html += '</div>';
      ...
      if (certifications.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.certifications) + '</div>';
        for (var cci = 0; cci < certifications.length; cci++) {
          ...
          var certLine = '';
          if (cert.name) certLine += '<span class="cert-name">' + escapeHtml(safeStr(cert.name)) + '</span>';
          if (cert.issuer) certLine += ' <span class="cert-detail">– ' + escapeHtml(safeStr(cert.issuer)) + '</span>';
          if (cert.date) certLine += ' <span class="cert-detail">(' + escapeHtml(formatDate(cert.date, lang)) + ')</span>';
          html += certLine;
          html += '</div>';
        }
        html += '</div>';
      ...
      if (languages.length > 0) {
        ...
        html += '<div class="section-title">' + escapeHtml(t.languages) + '</div>';
  ...
  if (!customElements.get('gqr-resume-mckinsey')) {
    customElements.define('gqr-resume-mckinsey', GQRResumeMckinsey);
  }
})();
