...
const parseLocation = (locale: string | { country?: string; language?: string }): { city?: string; country?: string } => {
  if (!locale) return {};
 ...
 */
export const verifyLinkedInToken = async (accessToken: string): Promise<LinkedInUserProfile> => {
  try {
 ...
 */
export const parseLinkedInProfile = (linkedinProfile: LinkedInUserProfile): {
  email: string;
  firstName: string;
  lastName: string;
  fullName: string;
 ...
 */
export const getFullProfileData = async (accessToken: string): Promise<Partial<ResumeData>> => {
  try {
