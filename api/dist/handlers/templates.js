...
exports.createTemplate = exports.listTemplates = void 0;
const rateLimiter_1 = require("../middleware/rateLimiter");
const templateService_1 = require("../services/templateService");
const listTemplates = async (event) => {
    try {
        ...
        const rl = await (0, rateLimiter_1.checkRateLimit)(userId, 'list-templates', 60, 60000);
        if (!rl.allowed) {
            return {
                statusCode: 429,
                headers: {
        ...
        const items = await templateService_1.templateService.listAll();
        const templates = await Promise.all(items.map(async (t) => ({
            id: t.id,
            name: t.name || 'Template',
            description: t.description,
            category: t.category,
            tagName: t.tagName || t.id, // Use tagName from DB or fallback to id
            jsCode: await templateService_1.templateService.getCode(t.s3Key),
            s3Key: t.s3Key,
            hash: t.hash,
        })));
        return {
...
const createTemplate = async (event) => {
    try {
        ...
        const rl = await (0, rateLimiter_1.checkRateLimit)(clientId, 'create-template', 30, 60000);
        if (!rl.allowed) {
            return {
                statusCode: 429,
                headers: {
