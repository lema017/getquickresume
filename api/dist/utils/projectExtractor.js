...
exports.extractProjectCandidates = extractProjectCandidates;
const MONTHS_ES = /(ene\.|feb\.|mar\.|abr\.|may\.|jun\.|jul\.|ago\.|sept\.|oct\.|nov\.|dic\.)/i;
const DATE_RANGE_REGEX = new RegExp(`(?:${MONTHS_ES.source}\\s?\\d{4}|\\d{4}(?:-\\d{2})?)\\s*-\\s*(?:actualidad|${MONTHS_ES.source}\\s?\\d{4}|\\d{4}(?:-\\d{2})?)`, 'i');
function isTitleLine(line) {
    ...
    if (/^(asociad[ao]\s+con|logotipo\s+de|technologies:|contributions:)/i.test(l))
        return false;
    if (/^[-•]/.test(l))
        return false; // bullets
    // Evitar que una fecha sea considerada título
    if (DATE_RANGE_REGEX.test(l))
        return false;
    return true;
}
function normalizeTechnologies(lines) {
    const techs = [];
...
function extractProjectCandidates(raw) {
    if (!raw || !raw.trim())
        return [];
    const lines = raw
        .split(/\r?\n/)
...
//# sourceMappingURL=projectExtractor.js.map