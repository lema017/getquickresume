...
function validateResumeScore(score) {
    ...
    const sanitized = {
        ...score,
        strengths: score.strengths.map((s) => s.trim().slice(0, 200)),
        improvements: score.improvements.map((i) => i.trim().slice(0, 200)),
        detailedFeedback: score.detailedFeedback.map((f) => ({
            ...f,
            section: f.section.trim().slice(0, 100),
            recommendations: f.recommendations.map((r) => r.trim().slice(0, 300)),
        })),
    };
