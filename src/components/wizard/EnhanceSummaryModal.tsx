...
import { X, Check, XCircle, Loader2, Wand2, ArrowRight } from 'lucide-react';
...
export function EnhanceSummaryModal({
  isOpen,
  onClose,
  originalText,
  type,
  ...
  const handleEnhance = async () => {
    ...
    try {
      let result: string;
      if (type === 'summary') {
        result = await experienceAchievementService.enhanceSummaryText(
          originalText,
          jobTitle,
          language,
          resumeId
        ...
        result = await experienceAchievementService.enhanceDifferentiatorsText(
          originalText,
          jobTitle,
          language,
          resumeId
  ...
  // Reset enhanced text when originalText changes
  useEffect(() => {
          ...
          {jobTitle && (
            <div className="mb-4 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                <span className="font-medium">{t('common.profession')}:</span> {jobTitle}
              </p>
            </div>
          )}
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-gray-600">
                {language === 'es' ? 'Mejorando texto con IA...' : 'Enhancing text with AI...'}
              </span>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2 flex items-center">
                  ...
                  {language === 'es' ? 'Texto Mejorado con IA' : 'AI Enhanced Text'}
                </h4>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <p className="text-sm text-green-800">
                  <strong>✨ {language === 'es' ? 'Mejoras aplicadas:' : 'Improvements applied:'}</strong>{' '}
                  {language === 'es' 
                    ? 'El texto ha sido optimizado con palabras clave relevantes, un tono más profesional y un enfoque en tus fortalezas únicas.'
                    : 'The text has been optimized with relevant keywords, a more professional tone, and a focus on your unique strengths.'
                  }
                </p>
              </div>
            </div>
          ...
          {!isLoading && !error && !enhancedText && (
            <div className="text-center py-8">
              <p className="text-gray-500">
                {language === 'es' ? 'No se pudo mejorar el texto.' : 'Could not enhance the text.'}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between p-6 border-t bg-gray-50 flex-shrink-0">
            <p className="text-sm text-gray-600">
              {language === 'es' 
                ...
                : 'Review the changes and decide if you want to apply the improvement'
              }
            </p>
            <div className="flex space-x-3">
              ...
              >
                ...
                <span>{language === 'es' ? 'Aprobar Mejora' : 'Approve Enhancement'}</span>
              </button>
            </div>
          </div>
        )}
