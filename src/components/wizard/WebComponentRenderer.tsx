  ...
  useLayoutEffect(() => {
    ...
    async function loadAndRender() {
      try {
        ...
        console.log('🔍 [WebComponentRenderer] Loading Web Component:', { tagName, codeLength: jsCode.length });
        ...
        console.log('🔍 [WebComponentRenderer] Element already defined?', !!elementDefined);
        ...
        // Fix templates that incorrectly assign to the read-only native shadowRoot getter.
        // `Element.prototype.shadowRoot` is a getter-only property; assigning to it
        // in strict mode (classes are always strict) throws a TypeError.
        interceptedCode = interceptedCode.replace(
          /this\.shadowRoot\s*=\s*null\s*;?/g,
          '/* shadowRoot init removed */'
        );
        interceptedCode = interceptedCode.replace(
          /this\.shadowRoot\s*=\s*this\.attachShadow/g,
          'this.attachShadow'
        );
        ...
        console.log('🔍 [WebComponentRenderer] TagName in code:', codeTagName, 'Expected:', tagName);
        
        // If the code has a different tagName than expected, replace it
        if (codeTagName && codeTagName !== tagName) {
          console.log('🔍 [WebComponentRenderer] Replacing tagName in code:', codeTagName, '→', tagName);
          ...
          console.log('🔍 [WebComponentRenderer] Code replaced successfully');
        }
        ...
        if (hasResumeComponent && originalClassName !== uniqueClassName) {
          console.log('🔍 [WebComponentRenderer] Replacing class name:', originalClassName, '→', uniqueClassName);
          ...
          console.log('🔍 [WebComponentRenderer] Class name replaced successfully');
        } else if (!hasResumeComponent) {
          console.log('🔍 [WebComponentRenderer] No ResumeComponent class found, skipping class name replacement');
        }
        
        const isAlreadyDefined = customElements.get(tagName);
        
        if (isAlreadyDefined) {
          // Modificar el código para que no intente definir el elemento si ya está registrado
          ...
          interceptedCode = interceptedCode.replace(definePattern, (match) => {
            // Reemplazar con una verificación que evita redefinir
            return `if (!customElements.get('${tagName}')) { ${match} }`;
          });
          console.log('🔍 [WebComponentRenderer] Intercepted code to avoid redefinition of', tagName);
          // Si ya está definido, no necesitamos cargar el script
        ...
        if (!elementDefined) {
          ...
          await new Promise<void>((resolve, reject) => {
            script.onload = () => {
              console.log('🔍 [WebComponentRenderer] Script loaded successfully');
              resolve();
            };
            script.onerror = (err) => {
              console.error('🔍 [WebComponentRenderer] Script load error:', err);
          ...
          console.log('🔍 [WebComponentRenderer] Waiting for custom element registration...');
          while (attempts < maxAttempts && !elementDefined && mounted) {
            await new Promise(resolve => setTimeout(resolve, 100));
            attempts++;
            elementDefined = customElements.get(tagName);
            if (elementDefined) {
              console.log('🔍 [WebComponentRenderer] Custom element registered after', attempts, 'attempts');
              break;
            }
            // Log all registered custom elements for debugging
            if (attempts % 10 === 0) {
              console.log('🔍 [WebComponentRenderer] Attempt', attempts, '- Checking for tagName:', tagName);
              ...
              try {
                ...
                console.log('🔍 [WebComponentRenderer] Found registered elements:', allElements);
              } catch (e) {
                console.log('🔍 [WebComponentRenderer] Could not list registered elements');
              }
            }
          }
          ...
          if (!elementDefined) {
            ...
            console.error('🔍 [WebComponentRenderer] Custom element not registered. Expected:', tagName);
            console.error('🔍 [WebComponentRenderer] Found registered elements:', foundTags);
            console.error('🔍 [WebComponentRenderer] Code snippet (first 500 chars):', interceptedCode.substring(0, 500));
