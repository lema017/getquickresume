...
import { Lock, TrendingUp, CheckCircle2, AlertCircle, AlertTriangle, Sparkles, RefreshCw, ChevronRight, Zap } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        {/* Free users: Show improvement hints and upgrade CTA */}
        {!isPremium && (
          <div className="border-t border-gray-200 pt-6">
            {/* Score Status Banner - Urgency indicator for low scores */}
            {score.totalScore < 7 && (
                ...
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-red-600">
                      Resumes scoring below 7/10 are often filtered out by ATS systems. Enhance yours to stand out.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {score.totalScore < 8 && (
              <div className="mb-5 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-medium text-gray-700">Score Potential</span>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-gray-400">{score.totalScore.toFixed(1)}</div>
            ...
            {/* Improvement Hints Section - Always show with fallback */}
            <div className="mb-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  <AlertTriangle className="w-5 h-5 text-amber-500 mr-2" />
                  <h4 className="text-sm font-semibold text-gray-900">{t('scoreCard.freeUser.areasToImprove')}</h4>
                </div>
                <button 
              ...
              {/* Improvement hints list */}
              <div className="space-y-2">
                {score.improvementHints && score.improvementHints.length > 0 ? (
                  score.improvementHints.map((hint, index) => (
                    <button
                      key={index}
                      onClick={() => setShowPremiumModal(true)}
                      className="w-full flex items-center justify-between p-3 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-100 hover:border-amber-300 transition-colors group"
                    >
                      <div className="flex items-center">
                        <AlertTriangle className="w-4 h-4 text-amber-500 mr-2 flex-shrink-0" />
                        <span className="text-sm text-gray-700">{hint}</span>
                      </div>
                      <div className="flex items-center text-purple-600 group-hover:opacity-100 opacity-70 transition-opacity">
                        <span className="text-xs font-medium mr-1">{t('scoreCard.freeUser.fix')}</span>
                        <ChevronRight className="w-4 h-4" />
                      </div>
                    </button>
                  ))
                ) : (
                  /* Fallback when no specific hints */
                  <>
                    <button
                      onClick={() => setShowPremiumModal(true)}
                      className="w-full flex items-center justify-between p-3 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-100 hover:border-amber-300 transition-colors group"
                    >
                      <div className="flex items-center">
                        <AlertTriangle className="w-4 h-4 text-amber-500 mr-2 flex-shrink-0" />
                        <span className="text-sm text-gray-700">Content optimization available</span>
                      </div>
                      <div className="flex items-center text-purple-600 group-hover:opacity-100 opacity-70 transition-opacity">
                        <span className="text-xs font-medium mr-1">{t('scoreCard.freeUser.fix')}</span>
                        <ChevronRight className="w-4 h-4" />
                    ...
                    >
                      <div className="flex items-center">
                        <AlertTriangle className="w-4 h-4 text-amber-500 mr-2 flex-shrink-0" />
                        <span className="text-sm text-gray-700">ATS compatibility improvements</span>
                      </div>
                      <div className="flex items-center text-purple-600 group-hover:opacity-100 opacity-70 transition-opacity">
                        <span className="text-xs font-medium mr-1">{t('scoreCard.freeUser.fix')}</span>
                        <ChevronRight className="w-4 h-4" />
            ...
            {/* Premium Benefits Quick List */}
            <div className="mb-5 grid grid-cols-2 gap-2">
              <div className="flex items-center p-2 bg-gray-50 rounded-lg">
                <CheckCircle2 className="w-4 h-4 text-green-500 mr-2 flex-shrink-0" />
                <span className="text-xs text-gray-600">Detailed checklists</span>
              </div>
              <div className="flex items-center p-2 bg-gray-50 rounded-lg">
                <CheckCircle2 className="w-4 h-4 text-green-500 mr-2 flex-shrink-0" />
                <span className="text-xs text-gray-600">AI enhancements</span>
              </div>
              <div className="flex items-center p-2 bg-gray-50 rounded-lg">
                <CheckCircle2 className="w-4 h-4 text-green-500 mr-2 flex-shrink-0" />
                <span className="text-xs text-gray-600">ATS optimization</span>
              </div>
              <div className="flex items-center p-2 bg-gray-50 rounded-lg">
                <CheckCircle2 className="w-4 h-4 text-green-500 mr-2 flex-shrink-0" />
                <span className="text-xs text-gray-600">Keyword analysis</span>
              </div>
            </div>

            {/* Upgrade CTA */}
            <div className="bg-gradient-to-r from-purple-600 via-violet-600 to-indigo-600 rounded-xl p-5 shadow-lg">
              <div className="text-center">
                ...
                <h4 className="text-lg font-semibold text-white mb-1">
                  {t('scoreCard.freeUser.enhanceTitle')}
                </h4>
                <p className="text-sm text-purple-100 mb-4">
                  {t('scoreCard.freeUser.enhanceDescription')}
                </p>
                <button
                  onClick={() => setShowPremiumModal(true)}
                  className="inline-flex items-center px-6 py-3 bg-white text-purple-700 font-semibold rounded-xl hover:bg-purple-50 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                >
                  <Zap className="w-5 h-5 mr-2" />
                  {t('scoreCard.freeUser.enhanceButton')}
                </button>
                <p className="text-xs text-purple-200 mt-3">
                  {t('scoreCard.freeUser.socialProof')}
                </p>
              </div>
            </div>
          </div>
