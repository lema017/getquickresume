...
export function TemplateSelectionModal({
  ...
  const handleSelect = async (templateId: string) => {
    ...
    if (loaded) {
      onSelectTemplate({ ...loaded, layout: 'single-column' });
    }
  };
                ...
                return (
                  <div
                    ...
                    className={`border rounded-lg p-4 bg-white hover:shadow-lg transition-all cursor-pointer ${
                      isSelected ? 'ring-2 ring-blue-500 border-blue-500 shadow-md' : 'border-gray-200'
                    }`}
                    onClick={() => handleSelect(meta.id)}
                    role="button"
                    tabIndex={0}
                    aria-label={`${t('resumeView.download.templateModal.selectTemplate')} ${meta.name}`}
                    onKeyDown={(e) => {
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); handleSelect(meta.id); }}
